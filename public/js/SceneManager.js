...
const LEVEL_COMPLETE_OVERLAY_TIME = 10;
const NUM_LEVELS = 1;
const CREDITS = "The End";
...
class SceneManager2 {
	...
	levelUpdate() {
		...
		if (this.boss && !this.boss.alive) {
			...
			this.sceneEntities.push(new Overlay(this.game, false, LEVEL_COMPLETE_OVERLAY_TIME));
			for (var i = this.droids.length - 1; i >= 0; i--) {
		...
		if (this.wonLevel) {
			...
			if (this.timeSinceBossDeath > LEVEL_COMPLETE_OVERLAY_TIME) {
				this.levelNumber++;
				if (this.levelNumber > NUM_LEVELS) {
	...
	startCreditsScene() {
		...
		this.initiallyPaused = false;
		this.sceneEntities = [];
		this.sceneEntities.push(new TextScreen(this.game, CREDITS));
		this.sceneEntities.push(new Overlay(this.game, true, LEVEL_TRANSITION_OVERLAY_TIME));
		this.startedFinalOverlay = false;
	}
